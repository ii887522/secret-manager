...
function updateExistingSecret() {
    ...
    showPagination(secretArray.map(secret => secret.key), {
        ...
        onSelected: index => {
            secrets.put(secretArray[index]?.key ?? '', questionNewPassword('Please enter a new secret > ', { keepWhitespace: true, charlist: '$< -~>', min: 16, max: 64, confirmMessage: 'Please reenter the new secret > ' }));
            console.log();
...
while (isRunning) {
    ...
    console.log('-----------------------------');
    switch (keyInSelect(['Save a new secret', 'Copy an existing secret', 'Read an existing secret', 'Update an existing secret', 'Delete an existing secret'], 'Please type a choice', { cancel: 'Exit' })) {
        case -1:
            isRunning = false;
            break;
        case 0:
